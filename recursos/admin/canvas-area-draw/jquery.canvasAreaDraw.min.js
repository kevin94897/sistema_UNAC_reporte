(function( $ ){
  ...
  var init = function(index, input, options) {
    ...
    draw = function(cond) {
      ...
      if (cond=='muestra') {
        ctx.fillStyle = 'rgba(7,134,232,0.3)';
      };
      ctx.fill();
      ctx.stroke();
    ...
    $('.reg-edificios').on('click','.dibujar',function(){
      points = $(this).attr('data-coordenadas').split(',').map(function(point) {
            return parseInt(point, 10);
          });
      draw('muestra');
    });
